...
const PROMPTS: Record<string, (lead: any) => string> = {
  cold_email: (l) =>
    `Skriv en professionell, kort och personlig kall e-post på svenska (max 150 ord) adresserad till beslutsfattaren på bolaget "${l.company_name}".
Bransch: ${l.industry || "okänd"}. Stad: ${l.city || "okänd"}. Bolagsform: ${l.company_form || "okänd"}. Registreringsdatum: ${l.registration_date || "okänt"}.
Referera till att bolaget nyligen registrerats och anpassa tonen efter branschen. Avsluta med en tydlig CTA att boka ett kort samtal.
Skriv bara e-postens innehåll, ingen ämnesrad.`,

  dm: (l) =>
    `Skriv ett kort LinkedIn-meddelande på svenska (max 60 ord) som känns personligt och inte säljigt.
Bolag: "${l.company_name}". Bransch: ${l.industry || "okänd"}. Stad: ${l.city || "okänd"}.
Fokus på att väcka nyfikenhet snarare än att sälja direkt. Skriv bara meddelandet.`,
