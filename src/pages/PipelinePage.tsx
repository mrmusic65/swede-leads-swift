...
import { fetchLeadNoteCounts, logLeadActivity } from '@/lib/lead-notes-api';
...
const STAGES: { id: PipelineStage; label: string; color: string; dotColor: string }[] = [
  { id: 'new', label: 'Ny', color: 'text-muted-foreground', dotColor: 'bg-muted-foreground' },
  { id: 'contacted', label: 'Kontaktad', color: 'text-blue-600', dotColor: 'bg-blue-500' },
  { id: 'meeting', label: 'Möte bokat', color: 'text-violet-600', dotColor: 'bg-violet-500' },
  { id: 'customer', label: 'Kund', color: 'text-emerald-600', dotColor: 'bg-emerald-500' },
  { id: 'not_interested', label: 'Ej intressant', color: 'text-red-600', dotColor: 'bg-red-500' },
];
...
// Droppable column
function DroppableColumn({ stageId, dotColor, color, label, children, count }: {
  stageId: string; dotColor: string; color: string; label: string;
  ...
  return (
    <div
      ref={setNodeRef}
      className={`flex flex-col min-w-[220px] w-full rounded-xl border transition-colors ${isOver ? 'border-primary/50 bg-primary/5' : 'border-border/50 bg-card/30'}`}
    >
...
// Lead card
function LeadCard({ company, score, noteCount, onClick, isDragging }: {
  company: Company; score: number; noteCount: number; onClick: () => void; isDragging?: boolean;
}) {
  return (
    <div className={`rounded-lg border bg-card p-3 transition-shadow ${isDragging ? 'shadow-lg opacity-80 rotate-1' : 'shadow-sm hover:shadow-md'}`}>
      ...
      {company.industry_label && <div className="mb-1.5"><IndustryBadge industry={company.industry_label} /></div>}
        ...
        {noteCount > 0 && (
          <Tooltip>
            <TooltipTrigger asChild>
              <span className="flex items-center gap-0.5"><MessageSquare className="w-3 h-3" />{noteCount}</span>
            </TooltipTrigger>
...
export default function PipelinePage() {
  ...
  const grouped = useMemo(() => {
    const map: Record<PipelineStage, Company[]> = { new: [], contacted: [], meeting: [], customer: [], not_interested: [] };
    filtered.forEach(c => {
      const stage = ((c as any).pipeline_stage || 'new') as PipelineStage;
      (map[stage] || map.new).push(c);
    });
    return map;
  }, [filtered]);
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (!company) return;
    const oldStage = ((company as any).pipeline_stage || 'new') as string;
    if (oldStage === newStage) return;

    setCompanies(prev => prev.map(c => c.id === companyId ? { ...c, pipeline_stage: newStage } as any : c));
    const { error } = await (supabase as any).from('companies').update({ pipeline_stage: newStage }).eq('id', companyId);
    if (error) {
  ...
  if (loading) {
    return (
      <div className="p-6 space-y-4">
        ...
        <div className="flex gap-4">{[1,2,3,4,5].map(i => <Skeleton key={i} className="h-[400px] flex-1 rounded-xl" />)}</div>
      </div>
    );
  }
    ...
    <TooltipProvider>
      <div className="flex flex-col h-full">
        ...
        <div className="flex flex-wrap items-center gap-3 px-6 py-4 border-b border-border/40">
          ...
          <ToggleGroup type="single" value={viewMode} onValueChange={v => v && setViewMode(v as any)} className="border rounded-lg p-0.5">
        ...
        {viewMode === 'kanban' ? (
          <DndContext sensors={sensors} collisionDetection={closestCorners} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
            <div className="flex-1 overflow-x-auto p-4">
              <div className="flex gap-3 min-w-max">
                {STAGES.map(stage => {
                  ...
                  return (
                    <DroppableColumn key={stage.id} stageId={stage.id} dotColor={stage.dotColor} color={stage.color} label={stage.label} count={items.length}>
                      {items.length === 0 ? (
                        ...
                        items.map(c => (
                          <DraggableCard key={c.id} id={c.id}>
                            <LeadCard company={c} score={scores[c.id] || 0} noteCount={noteCounts[c.id] || 0} onClick={() => setSelectedLead(c)} />
                          </DraggableCard>
                        ))
                      )}
                    </DroppableColumn>
            ...
            <DragOverlay>
              {activeCompany && (
                <LeadCard company={activeCompany} score={scores[activeCompany.id] || 0} noteCount={noteCounts[activeCompany.id] || 0} onClick={() => {}} isDragging />
              )}
            </DragOverlay>
          </DndContext>
        ) : (
                    ...
                    return (
                      <tr key={c.id} className="border-b border-border/20 hover:bg-muted/20">
                        <td className="px-4 py-3">
                          <button onClick={() => setSelectedLead(c)} className="font-medium text-foreground hover:text-primary hover:underline">{c.company_name}</button>
                        </td>
                        <td className="px-4 py-3"><span className={`text-xs font-medium ${stage?.color}`}>{stage?.label}</span></td>
        ...
        {selectedLead && (
          <LeadSlideOver company={selectedLead} open={!!selectedLead} onClose={() => setSelectedLead(null)} />
        )}
      </div>
    </TooltipProvider>
  );
